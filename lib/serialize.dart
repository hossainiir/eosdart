...
class Action {
    ...
    factory Action.fromJson(Map json){
      return Action(
        ...
        name: json["name"],
        data: json["data"],
        authorization: (json["authorization"] as List).map((item)=>Authorization.fromJson(item)).toList(),
      );
    }
}
...
int dateToBlockTimestamp(String date) {
    return ((checkDateParse(date + 'Z').subtract(Duration(milliseconds: 946684800000))).millisecondsSinceEpoch / 500).round();
}
  ...
  // try {
    ...
    for (var field in self.fields) {
        if (dy[field.name] != null) {
            ...
            field.type.serialize(field.type,
                buffer, dy[field.name], state:state, allowExtensions:allowExtensions && field == self.fields[self.fields.length - 1]);
        } else {
  ...
  try {
    ...
    for (var field in self.fields) {
        if (allowExtensions && field.type.extensionOf!=null && !buffer.haveReadData()) {
            ...
            result[field.name] = field.type.deserialize(field.type,buffer, state:state,allowExtensions: allowExtensions);
        }
    }
    return result;    
  } catch (e) {
...
serializeVariant(Type self,SerialBuffer buffer,Object data,{SerializerState state , allowExtensions = true}) {
  if(state == null) state = SerializerState();
    if (!(data is List) || (data as List).length != 2 || !((data as List)[0] is String)) {
...
class CreateTypeArgs {
    ...
    Type base;
    List<Field> fields;
    void Function(SerialBuffer buffer, Object data, {SerializerState state,bool allowExtensions}) serialize;
    int Function(SerialBuffer buffer , {SerializerState state,bool allowExtensions}) deserialize;
}
...
String serializeActionData(Contract contract ,String account,String name,Object data) {
...
SerializedAction serializeAction(Contract contract,String account,String name,
                                List<Authorization> authorization, dynamic data) {
...
Object deserializeActionData(Contract contract,String account,String name,Object data) {
    var action = contract.actions[name];
    if (data is String) {
    ...
    var buffer = SerialBuffer(Uint8List.fromList(data));
    return action.deserialize(action,buffer);
}

/// Deserialize action. If `data` is a `string`, then it's assumed to be in hex. */
Action deserializeAction(Contract contract,String account,String name,List<Authorization> authorization,Object data) {
    return Action(
        account:account,
        name:name,
        authorization:authorization,
